...
def readFile(file_path):
    binary_extensions = ['.jpg', '.png', '.img', '.jpeg', '.gif', '.bmp']
...
def browse_file():
    ...
    else:  return render_template('index.html',error="Wrong File Path.")
...
def encrypt_file():
    ...
    key, salt = generate_key_from_password(lock)
    ...
    else: return render_template('index.html',error="Something Went Wrong.")
...
def decrypt_file():
    ...
    try:
        ...
        if file_path and lock:
            ...
            key = load_key_from_password(lock, salt)
            fernet = Fernet(key) 
